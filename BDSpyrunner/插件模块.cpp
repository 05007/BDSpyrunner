...
#pragma endregion
#pragma region 全局变量
static VA Cmd_Queue, Level_, Server_Network_Handle;
...
// 模块声明
static PyModuleDef mc = { PyModuleDef_HEAD_INIT, "mc", 0, -1,m,0,0,0,0 };
#pragma endregion
#pragma region Hook
...
THook(玩家破坏方块, bool, "?_destroyBlockInternal@GameMode@@AEAA_NAEBVBlockPos@@E@Z",
	...
	Player* p = f(Player*, _this + 8);
	BlockSource* bs = f(BlockSource*, f(VA, _this + 8) + 800);
...
THook(玩家开箱准备, bool, "?use@ChestBlock@@UEBA_NAEAVPlayer@@AEBVBlockPos@@@Z",
...
THook(玩家开桶准备, bool, "?use@BarrelBlock@@UEBA_NAEAVPlayer@@AEBVBlockPos@@@Z",
	VA _this, Player* p, BlockPos* bp) {
	getPlayerInfo(p);
...
THook(玩家放入取出, void, "?containerContentChanged@LevelContainerModel@@UEAAXH@Z",
	VA a1, VA slot) {
	VA v3 = f(VA, a1 + 208);// IDA LevelContainerModel::_getContainer
	BlockSource* bs = f(BlockSource*, f(VA, v3 + 808) + 72);
